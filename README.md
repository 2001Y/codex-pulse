...
- optional SQLite state wiring now exists for trigger runs, deliveries, delivered-item suppression history, audit-derived feedback logs, approval/action logs for `action_prep` plus minimal approve/reject/complete/failed transitions, strict command/flag validation, and execution-details persistence for receipt/error notes, X-signal connector cursors, source-registry state snapshots (`last_poll_at`, `last_seen_item_ids`, `last_promoted_item_ids`, `authority_tier`), and same-trigger suppression filtering for digest delivery; richer source-registry state (for example trust-review notes/error metadata) and deeper action execution/feedback loops remain future work
