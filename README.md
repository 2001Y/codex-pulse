...
- optional SQLite state wiring now exists for trigger runs, deliveries, delivered-item suppression history, X-signal connector cursors, and source-registry state snapshots (`last_poll_at`, `last_seen_item_ids`, `last_promoted_item_ids`, `authority_tier`); feedback persistence and richer source-registry state (for example trust-review notes and error metadata updates) remain future work
