...
- optional SQLite state wiring now exists for trigger runs, deliveries, delivered-item suppression history, approval/action logs, audit-derived feedback logs, strict action/suppression command validation, structured execution-details persistence, action-execution feedback logging, X-signal connector cursors, source-registry state snapshots (`last_poll_at`, `last_seen_item_ids`, `last_promoted_item_ids`, `authority_tier`), structured source-registry notes, same-trigger suppression filtering for digest delivery, and minimal suppression dismiss/expiry transitions; deeper action execution/feedback loops still remain future work
