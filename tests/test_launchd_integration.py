...
def test_render_launchd_plist_serializes_label_schedule_and_program_arguments() -> None:
    plist = render_launchd_plist(
        LaunchdPlistSpec(
            ...
            program_arguments=["/Users/akitani/bin/run-codex-pulse.sh"],
            hour=8,
            minute=15,
            working_directory=REPO_ROOT,
            standard_out_path=Path("/tmp/codex-pulse.out.log"),
            standard_error_path=Path("/tmp/codex-pulse.err.log"),
        )
    )

    payload = plistlib.loads(plist.encode())

    assert payload["Label"] == "dev.nous.codex-pulse.codex-delivery"
    assert payload["ProgramArguments"] == ["/Users/akitani/bin/run-codex-pulse.sh"]
    assert payload["StartCalendarInterval"] == {"Hour": 8, "Minute": 15}
    assert payload["WorkingDirectory"] == str(REPO_ROOT)
    assert payload["StandardOutPath"] == "/tmp/codex-pulse.out.log"
    assert payload["StandardErrorPath"] == "/tmp/codex-pulse.err.log"
...
def test_generate_launchd_artifacts_writes_wrapper_and_plist_to_output_directory(tmp_path: Path) -> None:
    ...
    assert artifacts == GeneratedLaunchdArtifacts(
        wrapper_path=output_directory / "run-codex-pulse-direct-delivery.sh",
        plist_path=output_directory / "dev.nous.codex-pulse.codex-delivery.plist",
    )
