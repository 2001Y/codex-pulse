...
- optional な SQLite state wiring は trigger runs / deliveries / X signal connector cursors / source registry state snapshots（`last_poll_at` / `last_seen_item_ids` / `last_promoted_item_ids` / `authority_tier`）まで入り、suppression / feedback persistence や、trust-review notes・error metadata 更新のようなより豊かな source registry state は今後の実装対象
