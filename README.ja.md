...
- optional な SQLite state wiring は trigger runs / deliveries / delivered-item suppression history / X signal connector cursors / source registry state snapshots（`last_poll_at` / `last_seen_item_ids` / `last_promoted_item_ids` / `authority_tier`）まで入り、feedback persistence や、trust-review notes・error metadata 更新のようなより豊かな source registry state は今後の実装対象
